...
const TopArtists = () => {
    ...
    const fetchTopArtists = async () => {
      try {
        const res = await fetch(
          'https://discoveryprovider.audius.co/v1/tracks/trending?limit=50&app_name=spotify-clone'
        );
        const data = await res.json();
        ...
        data.data.forEach((track) => {
          const artist = track.user;
          if (artist && artist.user_id && !artistMap[artist.user_id]) {
            artistMap[artist.user_id] = artist;
          }
        });

        const uniqueArtists = Object.values(artistMap);
        console.log('Unique artist count:', uniqueArtists.length);
  ...
  if (isLoading) return <Loader title="Loading Top Artists" />;
    ...
    <div className="flex flex-col">
      ...
      <div className="flex flex-wrap sm:justify-start justify-center gap-8">
        {topArtists.map((artist) => (
          <ArtistCard key={artist.user_id} track={artist} />
        ))}
      </div>
    </div>
  );
