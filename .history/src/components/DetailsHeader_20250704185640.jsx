...
import { useGetArtistQuery } from '../redux/services/audiusCoreApi';


const DetailsHeader = ({artistId, songData }) => {
  const { data: artistData } = useGetArtistQuery(artistId);

  console.log('artist image URL:', artistData?.data?.profile_picture?.['480x480'] || artistData?.data?.profile_picture?.['150x150']);
