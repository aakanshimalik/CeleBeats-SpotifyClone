...
const TopChartCard = ({artist, i})=> (
  <div className='w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2'>
    ...
    <diiv className="flex-1 flex flex-row justify-between items-center">
      <img className='w-20 h-20 rounded-lg' src={artist?.artwork?.['150x150']} alt={artist?.title}/>
      ...
      <div ref={divRef} className='xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col'>
        ...
        >
          {topPlays?.slice(0, 5).map((artist,i) => (
            ...
            <SwiperSlide
            key={artist?.user?.id || i}
            style={{ width: '25%', height: 'auto' }}
            className="shadow-lg rounded-full animate-slideright"
            >
              <Link to={`/artists/${artist?.user?.id}`}>
              <img src={
                artist?.user?.profile_picture?.['150x150']  ||  artist?.user?.defaultProfile
