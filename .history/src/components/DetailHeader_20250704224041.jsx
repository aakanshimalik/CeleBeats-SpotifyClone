  ...
  <div className='relative w-full flex flex-col'>
    ...
    <div className='absolute inset-0 flex items-center'>
      ...
      <div className="ml-5">
        ...
        {artistId ? artistData?.user?.name || artistData?.user?.handle : songData?.title}
        {/* {songData?.title || 'Unknown Title'} */}
        </p>
        {!artistId && (
          <Link to={`/artists/${artistData?.user?.id}`}>
            <p className="text-base text-gray-400 mt-2">{songData?.user?.name || artistData?.user?.handle}</p>
          </Link>
        )}
